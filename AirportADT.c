...
void freeAirport(AirportADT airport){                                       
	...
	free(airport->oaci);
	free(airport->iata);
	free(airport->denomination);
	freeMovementList(airport->movements);
	free(airport);
}
...
AirportList newAirportList(){
	return newList( (OACI (*) (void *)) getAirportOACI, (void (*) (void *)) freeAirport);
}
...
void freeAirportList(AirportList list){
	freeList(list);
}